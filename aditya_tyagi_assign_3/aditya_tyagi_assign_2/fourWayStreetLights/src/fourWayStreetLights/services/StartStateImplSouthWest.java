...
public class StartStateImplSouthWest implements StreetLightsStateI {
	...
	public void move(int cntCar) {
		...
		if(cntCar == 1) {
			System.out.println("Only one car for intersection.It will be crossed from the South. So, Traffic Light at the West is changed to Red");
...
//			streetLightsContext.noOfCarAtIntersection(cntCar);
			...
			results.writeToFile("Only one car for intersection.So, Traffic Light at the West is changed from green to  Red and car is crossed from the South. After crossing, traffic light at the South changes from green to red. Number of cars left after intersection: "+(cntCar-1));
...
//			streetLightsContext.noCarAtIntersection.move(cntCar);
			System.out.println("number of Cars left after Intersection: "+streetLightsContext.noOfCarAtIntersection(cntCar));
			results.writeToFile("Both cars are crossed from the South. No need to pass them from the West.Traffic Light at the West is changed to RED.After crossing, Traffic Light at the South is changed to RED. Number of cars left after intersection: "+cntCar);
...
//				streetLightsContext.noCarAtIntersection.move(cntCar);
				System.out.println("number of Cars left after Intersection: "+streetLightsContext.noOfCarAtIntersection(cntCar));
				results.writeToFile("Two cars are croosed from the South and the traffic light at the South is turned from green to red. Only one car is crossed from the west and its traffic light is turned from green to red. Number of cars left after intersection : "+cntCar);
				...
				System.out.println("Traffic Light at The West is Changed to Red");
				System.out.println("number of Cars left after Intersection: "+streetLightsContext.noOfCarAtIntersection(cntCar));
				results.writeToFile("Two cars are croosed from the South and the traffic light at the South is turned from green to red. Two cars are crossed from the west and its traffic light is turned from green to red. Number of cars left after intersection: "+cntCar);
